...
import { insertUserSchema, insertSiteConfigSchema, insertTestimonialSchema, insertFaqItemSchema, insertServiceSchema, insertPhotoCarouselSchema, insertExpertiseCardSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Expertise Cards routes (formerly Specialization Areas)
  app.get("/api/specialization-areas", async (req, res) => {
    try {
      const areas = await storage.getActiveExpertiseCards();
      res.json(areas);
    } catch (error) {
      res.status(500).json({ error: "Erro interno do servidor" });
    }
  ...
  app.get("/api/admin/specialization-areas", async (req, res) => {
    try {
      const areas = await storage.getAllExpertiseCards();
      res.json(areas);
    } catch (error) {
      res.status(500).json({ error: "Erro interno do servidor" });
    }
  ...
  app.post("/api/admin/specialization-areas", async (req, res) => {
    try {
      const areaData = insertExpertiseCardSchema.parse(req.body);
      const area = await storage.createExpertiseCard(areaData);
      res.json(area);
    } catch (error) {
      res.status(400).json({ error: "Dados inválidos" });
    }
  ...
  app.put("/api/admin/specialization-areas/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const areaData = req.body;
      const area = await storage.updateExpertiseCard(id, areaData);
      res.json(area);
    } catch (error) {
      res.status(400).json({ error: "Dados inválidos" });
    }
  ...
  app.delete("/api/admin/specialization-areas/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
