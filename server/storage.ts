...
import { type User, type InsertUser, type AdminUser, type InsertAdminUser, type SiteConfig, type InsertSiteConfig, type Testimonial, type InsertTestimonial, type FaqItem, type InsertFaqItem, type Service, type InsertService, type PhotoCarousel, type InsertPhotoCarousel, type ExpertiseCard, type InsertExpertiseCard } from "@shared/schema";
import { db } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllExpertiseCards(): Promise<ExpertiseCard[]> {
    return await db.select().from(expertiseCards).orderBy(asc(expertiseCards.order));
  }
  ...
  async deleteExpertiseCard(id: number): Promise<void> {
    await db.delete(expertiseCards).where(eq(expertiseCards.id, id));
  }
}
